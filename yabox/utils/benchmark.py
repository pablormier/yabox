    ...
    if method == 'scipy_de':
        ...
        if method == 'yabox_de':
            it = DE(problem, problem.bounds, popsize=popsize, maxiters=maxiters, mutation=mutation,
                    crossover=recombination, self_adaptive=False).geniterator()
...
def run_default(problems, methods=None, runs=1, popsize=64,
                stop_after=30, maxiters=1000000, mutation=0.5, recombination=0.5):
    if methods is None:
...
def test(function, config):
    ...
    return run_default(problems, **config)
